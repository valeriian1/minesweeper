...
def mock_pygame():
    with patch('src.ui.assets.pygame') as mock_pg:
        # Створюємо мок-поверхню, яка буде повертатись
        # замість реальних зображень
...
def test_asset_manager_init(mock_pygame):
    ...
    # Перевіряємо, що спрайти дошки завантажились правильно
    # (__init__ викликає _load_board_sprites)
...
def test_load_frames(mock_pygame, name_prefix, start_idx, count):
    """
    Перевіряємо чи функція _load_frames правильно завантажує
      очікувану кількість кадрів.
    Використовуємо параметризацію для тестування з різним
      префіксом і кількістю.
    """
...
def test_load_board_sprites(mock_pygame):
    """
    Перевіряємо чи всі необхідні клітинки ігрового
    поля завантажились і промасштабувались.
    """
    ...
    assert len(board) == 12
    # Функція pygame.transform.scale мала бути викликана 12
    # разів з розміром (25, 25)
